# -*- coding: utf-8 -*-
"""
Created on Wed Feb 13 09:11:18 2019
...
for j in range(len(Q2)):
    ...
    for k in range(len(A3)): 
        for l in range(len(S3)):
                price_change =[1,-1]
                price_probabilities =[0.5,0.5]
                P4[l,:] = P3[l]+np.random.choice(price_change,N,p=price_probabilities)
                demand_change =[1,-1]
                demand_probabilities =[0.5,0.5]
                D4[l,:] = D3[l]+np.random.choice(demand_change,N,p=demand_probabilities)
                for i in range(N):
    ...
    for k in range(len(A3)):
        for l in range(len(S3)): 
            for i in range(N): 
                Q3[l,k,:] = [S3[l,0],S3[l,1],S3[l,2],S3[l,3],A3[k],Q3[l,k,5] + alpha *(0 + gamma*Q4[l,i,k,5] - Q3[l,k,5])]
                alpha = 1/(i+1)

    # Compute Q2 based on Q3   
    for l in range(N):
        Q2[j,:] = [S2[0],S2[1],S2[2],S2[3],A2,Q2[j,5]+alpha*(0+gamma*max(Q3[l,:,5]) - Q2[j,5])]
        alpha = 1/(i+1)
